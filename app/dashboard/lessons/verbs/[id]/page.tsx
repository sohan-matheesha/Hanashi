...
import { verbsLessons, VerbsItem } from '../data'
import { notFound } from 'next/navigation'
import Link from 'next/link'
import { ChevronRight } from 'lucide-react'

function BasicCard({ char, className = "" }: { char: VerbsItem, className?: string }) {
  if (!char) return null;
  return (
...
export default function VerbsItemPage({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = use(params)
  const lesson = verbsLessons.find(l => l.id === resolvedParams.id)
