...
import { hiraganaLessons, HiraganaCharacter, LessonModule } from '../../data'
import { ChevronRight } from 'lucide-react'
import { notFound } from 'next/navigation'

function BasicCard({ char, className = "" }: { char: HiraganaCharacter, className?: string }) {
  const [showWord, setShowWord] = useState(false);

  if (!char) return null;
  return (
    <div className={`bg-white rounded-4xl p-6 pb-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 flex flex-col hover:border-pink-200 transition-colors group h-full ${className}`}>
      ...
      <div className="w-full border-t border-gray-100 pt-4 mt-auto cursor-pointer" onClick={() => setShowWord(!showWord)}>
        <p className="text-[8px] font-black text-gray-400 tracking-widest uppercase mb-1.5 flex items-center justify-between">
          Example Word
          <span className="text-[8px] text-pink-400 opacity-0 group-hover:opacity-100 transition-opacity bg-pink-50 px-2 py-0.5 rounded-full">{showWord ? 'Hide' : 'Reveal'}</span>
        </p>
        <div className={`transition-all duration-300 origin-left ${showWord ? 'opacity-100 blur-none scale-125 pt-2 pb-1' : 'opacity-80 blur-sm select-none'}`}>
...
function Section3({ lesson }: { lesson: LessonModule }) {
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        {lesson.content.map((char: HiraganaCharacter, i: number) => (
...
function Section4({ lesson }: { lesson: LessonModule }) {
  // Map characters by Romaji to place them precisely
  const charMap: Record<string, HiraganaCharacter> = lesson.content.reduce((acc: Record<string, HiraganaCharacter>, char: HiraganaCharacter) => {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 auto-rows-auto pb-8">
        ...
        <div className="md:row-span-2 col-span-1 bg-[#ece9f0] rounded-4xl p-6 pb-8 shadow-sm flex flex-col items-center">
        ...
        {/* ME large block */}
        <div className="md:col-span-2 bg-[#46548a] rounded-4xl p-8 shadow-md flex flex-col md:flex-row h-full relative overflow-hidden text-white min-h-55">
...
function Section5({ lesson }: { lesson: LessonModule }) {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-12 flex justify-between">
        ...
        <div className="w-[35%] h-45">
          <div className="w-full h-full rounded-4xl bg-linear-to-tr from-[#90733a] to-[#d9a857] shadow-xl overflow-hidden relative border-4 border-[#fffbeb]">
             {/* Simulated video/ink block */}
             <div className="absolute right-0 bottom-0 top-0 w-1/2 bg-black/10"></div>
             <div className="absolute right-8 top-1/2 -translate-y-1/2">
               <div className="w-2 h-16 bg-black/15 rounded-full mx-auto"></div>
             </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#f0fcfe] rounded-4xl p-8 mb-4 border border-[#d2eff2] flex flex-col md:flex-row items-center gap-8">
        ...
        <div className="md:w-2/3 flex gap-2 justify-between flex-wrap">
          {dakuon.map((d: HiraganaCharacter) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-10">
        ...
        {/* Re spans 2 columns */}
        <div className="md:col-span-2 bg-[#46548a] rounded-4xl p-8 shadow-md flex flex-col justify-between relative overflow-hidden text-white h-80">
...
function SectionDefault({ lesson }: { lesson: LessonModule }) {
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-12">
...
export default function HiraganaCharacterPage({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = use(params)
  const lesson = hiraganaLessons.find((l: LessonModule) => l.id === resolvedParams.id)
